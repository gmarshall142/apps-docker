...
function processLine(line) {
  return new Promise((resolve) => {
    ...
    if (elements[1] !== undefined && elements[1] !== '') {
      ...
      const ml = elements[2].replace(/"/g, '');
      const af = Boolean(Number(elements[3].replace(/"/g, '')));
      const descr = elements[4].replace(/"/g, '');
      const active = Boolean(elements[5]);
      const display = name + ' ' + descr;
      findWorkCenter(name)
        .then((resp) => {
          if (resp === undefined) {
